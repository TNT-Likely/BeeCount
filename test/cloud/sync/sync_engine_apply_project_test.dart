...
void main() {
  ...
  test('远端 transaction 省略 link + 保留孤儿 link → 完整行不变', () async {
    ...
    await (db.update(db.transactions)..where((t) => t.id.equals(txId))).write(
      const TransactionsCompanion(
        projectBudgetSyncId: Value('missing-retained-project'),
      ),
    );
  ...
  test('远端 transaction 省略 link + 保留跨账本 project → 完整行不变', () async {
    ...
    await (db.update(db.transactions)..where((t) => t.id.equals(txId))).write(
      const TransactionsCompanion(
        projectBudgetSyncId: Value('project-retained-cross-ledger'),
      ),
    );
    final before =
  ...
  test('远端 transaction 省略 link + 保留同账本 total → 完整行不变', () async {
    ...
    await (db.update(db.transactions)..where((t) => t.id.equals(txId))).write(
      const TransactionsCompanion(
        projectBudgetSyncId: Value('retained-total'),
      ),
    );
  ...
  test('malformed project field types are skipped and valid sibling survives',
    ...
    final variants = <String, Map<String, dynamic>>{
      'proj-wrong-type': {...validProject('proj-wrong-type'), 'type': 42},
      'proj-wrong-amount': {
