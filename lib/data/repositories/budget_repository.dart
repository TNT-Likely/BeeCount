...
abstract class BudgetRepository {
  ...
  /// v31: 恢复路径专用 — 按 syncId 直接 upsert budget 行,**不经**
  /// LocalRepository wrapper 的 changeTracker 记录。已有 syncId → 更新;不存在
  /// → 插入。返回本地 int id。
  ///
  /// 与 [createBudget] 的差异:createBudget 会生成新 UUID 且记 change;这里
  /// 保留 payload 中的 syncId,并由**调用方**决定是否记 change(见 import 侧
  /// [DataImportService.importBudgets] 的 recordChanges 分支)。
  ///
